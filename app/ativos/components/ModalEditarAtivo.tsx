  ...
  return (
    <Dialog open={aberto} onOpenChange={aoFechar}>
      <DialogContent className="max-w-2xl max-h-screen overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold mb-4">Editar Ativo</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              {errors.fabricante && <p className="text-sm text-red-500">{errors.fabricante.message}</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.data_aquisicao && <p className="text-sm text-red-500">{errors.data_aquisicao.message}</p>}
            </div>

            <div className="space-y-2">
              <Label htmlFor="localizacao_interna">Localização Interna</Label>
              <Input id="localizacao_interna" {...register("localizacao_interna")} />
              {errors.localizacao_interna && <p className="text-sm text-red-500">{errors.localizacao_interna.message}</p>}
            </div>

            <div className="space-y-2">
              <Label htmlFor="tipo_ativo">Tipo de Ativo</Label>
              <Input id="tipo_ativo" {...register("tipo_ativo", { required: true })} />
              {errors.tipo_ativo && <p className="text-sm text-red-500">{errors.tipo_ativo.message}</p>}
            </div>

            <div className="space-y-4">
              <Label htmlFor="foto">Imagem (URL)</Label>
              <Input id="foto" placeholder="https://exemplo.com/imagem.jpg" {...register("foto")} />
            </div>
          </div>
