...
export default function FormularioSistema() {
 const { register, handleSubmit, setValue, formState: { errors, isSubmitting }, reset } = useForm<SistemaItf>();
   ...
   const onSubmit = async (data: SistemaItf) => {
     try {
       setApiError(null);
       const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/sistema`, {
         method: "POST",
         headers: {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="space-y-4">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          {errors.id_area && <p className="text-sm text-red-500">{errors.id_area.message}</p>}
          </div>
        </div>
      </div>
